  ...
  constructor(web3: Web3, address?: string) {
        super(address ? new web3.eth.Contract(EnergyCertificateBundleDBJSON.abi, address) : new web3.eth.Contract(EnergyCertificateBundleDBJSON.abi, EnergyCertificateBundleDBJSON.networks.length > 0 ? (EnergyCertificateBundleDBJSON.networks[0]) : null));
        this.web3 = web3;
    }
  ...
  async createEnergyCertificateBundle(_tradableEntity: any, _certificateSpecific: any, txParams?: SpecialTx) {

            let transactionParams;

            const txData = await this.web3Contract.methods.createEnergyCertificateBundle(_tradableEntity, _certificateSpecific)
            .encodeABI();
            ...
            if (txParams) {
                ...
                if (!txParams.gas) {
                    ...
                    try {
                    gas = await this.web3Contract.methods.createEnergyCertificateBundle(_tradableEntity, _certificateSpecific)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} );
                    } catch (ex) {
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.createEnergyCertificateBundle(_tradableEntity, _certificateSpecific)
                    .send({from: transactionParams.from, gas: transactionParams.gas});
            }
    }
